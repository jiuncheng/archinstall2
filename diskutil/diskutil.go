...
func (d *DiskUtil) FormatDiskToGPT() error {
	return cmd.NewCmd("sgdisk -o " + d.cfg.InstallDisk).SetDesc("Formatting selected disk...").Run()
}

func (d *DiskUtil) NewEFIPartition() error {
	return cmd.NewCmd("sgdisk -n 1:0:+500M -t 1:ef00 -c 1:'EFI' " + d.cfg.InstallDisk).Run()
}

func (d *DiskUtil) NewLinuxFSPartition(size string) error {
	return cmd.NewCmd("sgdisk -n 2:0:" + size + " -t 2:8300 -c 2:'rootfs' " + d.cfg.InstallDisk).Run()
}

func (d *DiskUtil) MkfsVfatEFI() error {
	return cmd.NewCmd("mkfs.vfat -n EFI " + d.cfg.InstallDisk + "1").Run()
}

func (d *DiskUtil) MkfsBTRFS() error {
	return cmd.NewCmd("mkfs.btrfs -f -L ROOT " + d.cfg.InstallDisk + "2").Run()
}
